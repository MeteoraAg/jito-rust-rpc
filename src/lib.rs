...
pub const BASE_URL: &str = "https://mainnet.block-engine.jito.wtf/api/v1";
...
pub struct BundleTipService;

#[derive(Deserialize, Clone, Copy)]
pub struct BundleRecommendedTips {
...
impl BundleTipService {
    pub async fn get_tips_amount() -> Result<BundleRecommendedTips> {
        ...
        let tips: Vec<BundleRecommendedTips> = response.json().await?;

        let tip = tips.first().context("No tips amount returned")?;
